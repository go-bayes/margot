...
analysis artefacts. Many lab projects accumulated archives of `.qs` files
this way.
...
If `library(qs)` works on your machine, the simplest route is a single
call:
...
This walks the directory recursively, reads each `.qs` with `qs::qread()`,
writes a `.qs2` sibling with `qs2::qs_save()`, and does an
`identical()` read-verify. Originals are kept by default. Re-run with
`delete_qs = TRUE` once you're satisfied.
...
deletion, so the safety net stays in place even with `delete_qs = TRUE`.
