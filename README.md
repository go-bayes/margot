...
Effects (ATT, ATE). It also supports workflows for Heterogeneous
Treatment Effects (CATE) using `grf`, and Longitudinal Modified
Treatment Policies using `lmtp`. `margot` is greatly indebted to the
authors of these two excellent packages. `margot` is best seen as an
extensive system of graphical and reporting functions that assist with
the challenging tasks of asking and answering causal questions
precisely.
