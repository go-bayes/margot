...
margot_assess_overlap <- function(model_results,
  ...
  # get exposure label from mapping if available
  exposure_label <- exposure_name
  if (!is.null(label_mapping) && exposure_name %in% names(label_mapping)) {
    exposure_label <- label_mapping[[exposure_name]]
  }
  ...
  if (plot) {
    p <- create_propensity_plot(W, W_hat, exposure_label)
...
#' @keywords internal
create_propensity_plot <- function(W, W_hat, exposure_name) {
