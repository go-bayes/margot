...
test_that("continuous-outcome multi-bias E-values use the internal conversion", {
  ...
  expected <- EValue::multi_evalues.RR(
    biases,
    est = margot:::.margot_ols_to_rr(corrected$`E[Y(1)]-E[Y(0)]`, sd = 1),
    lo = margot:::.margot_ols_to_rr(corrected$`2.5 %`, sd = 1),
    hi = margot:::.margot_ols_to_rr(corrected$`97.5 %`, sd = 1),
    true = 1
  )
...
test_that("disabling Bonferroni retains the supplied confidence limits", {
  input <- data.frame(
    `E[Y(1)]-E[Y(0)]` = 0.2,
    `2.5 %` = 0.05,
    `97.5 %` = 0.35,
...
test_that("supplied multiplicity controls the Bonferroni adjustment", {
  input <- data.frame(
    `E[Y(1)]-E[Y(0)]` = 0.2,
    `2.5 %` = 0.05,
    `97.5 %` = 0.35,
