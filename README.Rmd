...
Use `margot_plot_ate()`, `margot_table_ate()`, and `margot_interpret_ate()` with saved `scale_info` to report model-scale estimates and their affine original-unit companions. The [outcome-scale guide](https://go-bayes.github.io/margot/articles/ate-reporting-scales.html) demonstrates this convention for weekly exercise. General-purpose log and log1p reporting remain available for historical analyses and other questions whose outcomes are explicitly defined on those scales. The preparation convention does not change an existing study's specification or remove those functions.
