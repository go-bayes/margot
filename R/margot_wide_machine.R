...
margot_wide_machine <- function(.data,
  ...
  for (t in 0:(y_ - 2)) {  # Excluding the final wave
    ...
    # column to move
    move_col <- paste0(t_prefix, exposure_var)
    ...
    data_wide_ordered <- data_wide_ordered %>%
      dplyr::relocate(dplyr::any_of(move_col), .after = dplyr::any_of(names(data_wide_ordered)[last_col_position]))
  }
...
# if we wanted not lost following wave we'd use this
# margot_wide_machine <- function(.data,
#                                 id = "id",
#                                 wave = "wave",
#                                 baseline_vars,
