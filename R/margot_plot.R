...
transform_to_original_scale <- function(results_df, original_df, label_mapping = NULL) {
  ...
  for (i in 1:nrow(results_df)) {
    ...
    if (full_col %in% names(original_df)) {
      cli::cli_alert_success("Found matching column in original data: {full_col}")
      ...
      cli::cli_alert_success("Transformation complete for {row_name}")
    } else {
      cli::cli_alert_warning("No matching column found in original data for {full_col}")
    }
  }

  cli::cli_alert_success("Transformation to original scale completed")
