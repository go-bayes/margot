...
margot_causal_forest <- function(data, outcome_vars, covariates, W, weights, grf_defaults = list(),
  ...
  for (outcome in outcome_vars) {
    ...
    tryCatch({
      ...
      # Pass the model to compute_qini_curves_binary so DR scores can be extracted
      qini_result <- compute_qini_curves_binary(model, tau_hat, Y, W)
...
#' Effect (ATE) Qini curves. A note is printed indicating that DR scores are used.
#'
...
compute_qini_curves_binary <- function(model, tau_hat, Y, W, verbose = TRUE) {
  tryCatch({
    ...
    dr_scores <- grf::get_scores(model)
    # Convert DR scores to a 2-column matrix: control and treatment outcomes.
    DR_matrix <- cbind(dr_scores[, "dr.y.hat.0"], dr_scores[, "dr.y.hat.1"])
    cost <- 1

    if (verbose) {
      cli::cli_alert_info(paste("DR scores dimensions:", paste(dim(DR_matrix), collapse = "x")))
    }

    # Calculate CATE Qini curve using DR scores
    cate_qini <- maq::maq(tau_hat, cost, DR_matrix, R = 200)
    # Calculate ATE Qini curve using DR scores
    ate_qini <- maq::maq(rep(mean(tau_hat), length(tau_hat)), cost, DR_matrix, R = 200)
