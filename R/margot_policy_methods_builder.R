...
margot_build_method_explanation <- function(stability, best, summary, context, citations = TRUE) {
  ...
  s2 <- paste0(
    ...
    cite("[@athey2021; @policytree_package_2024]")
  )
  ...
  s6 <- paste0(
    "We report effect sizes and uncertainty without dichotomous rules, and show large-but-uncertain 'signals worth monitoring' ranked by magnitude relative to uncertainty (SNR-style), optionally adjusted for coverage and stability",
    cite("[@efron2012large; @tusher2001significance; @smyth2004linear; @gelman2006difference]"), "."
  )
  ...
  if (!is.null(context$signal_score) && !identical(context$signal_score, "none")) {
    ...
    base <- switch(sc,
      ...
      hybrid = "Signal score (hybrid) is 0.7 * PV-SNR + 0.3 * uplift-SNR, weighting policy value more heavily while still showing large uplift signals.",
      "Signal score ranks magnitude relative to uncertainty and applies coverage and stability weights."
    )
    refs <- cite("[@efron2012large; @tusher2001significance; @smyth2004linear; @gelman2006difference; @meinshausen2010stability]")
    clarification <- " This descriptive ranking shows potentially actionable heterogeneity that warrants further investigation."
    s8 <- paste0(base, " Shown as a descriptive ranking (no gating); top ", context$signals_k %||% 3, " are reported in text.", clarification, refs)
  }
  ...
  prereg <- if (!is.null(prereg_bp) && nzchar(prereg_bp)) prereg_bp else paste0(
    ...
    "7) Reporting: no dichotomous thresholds; show large-but-uncertain signals using magnitude/uncertainty rankings (", context$signal_score %||% "none", ").\n",
