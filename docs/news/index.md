...
- Policy learning accepts explicit benefit thresholds, including signed
  fixed values and a development-sample ATE reference. Treatment rewards
  subtract the threshold before analysis weights are applied once. Tree
  learning and constant comparisons use the same net-value objective;
  original effects and values remain available. Existing CV and display
  calls retain their zero-threshold default and legacy scientific
  results.

- [`margot_policy_development_scores()`](https://go-bayes.github.io/margot/reference/margot_policy_development_scores.md)
  fits outcome, exposure and causal forests using development
  participants, producing out-of-bag development scores and
  development-trained evaluation predictions.
  [`margot_policy_action_scores()`](https://go-bayes.github.io/margot/reference/margot_policy_action_scores.md)
  constructs binary doubly robust action scores without applying weights
  or thresholds. Invalid propensities fail rather than being silently
  clipped.

- [`margot_policy_tree_evaluate()`](https://go-bayes.github.io/margot/reference/margot_policy_tree_evaluate.md)
  learns a threshold, constant comparator and shallow rule in
  development, then evaluates that same rule on separate supplied
  scores. It reports original and net values, leaf contrasts,
  paired-score differences and nominal pointwise intervals conditional
  on the learned rule and supplied preparation. These intervals require
  appropriate sampling and nuisance-estimation assumptions and do not
  quantify every source of preparation or population-projection
  uncertainty. Existing stored-score CV remains a different evaluation
  mode.
...
- [`margot_policy_evaluation_reporting_data()`](https://go-bayes.github.io/margot/reference/margot_policy_evaluation_reporting_data.md)
  binds evaluated rules to stored reporting.
  `reporting_layout = "two_panel"` supplies caption-free tree and
  weighted-projection artwork while retaining standalone leaf and value
  evaluation plots and text. Labels distinguish original effects from
  net value and below-threshold benefit from harm. An estimated ATE is a
  relative reference; positive targeting value under that realised
  threshold does not by itself establish heterogeneous effects.

- Policy decision trees accept a native `policytree` object without a
  causal-forest wrapper. Branch labels can show the fitted inequalities,
  use a named left/right pair, or use per-edge tables or callbacks.
  Existing supplied leaf labels retain their separate assignment and
  outcome-contrast meanings. Custom titles are now printed literally,
  without title-casing or an appended “Outcome”.

- `layout_style = "compact"` reduces decision-tree margins and title
  spacing, centres parents over ordered children, and uses the available
  panel aspect ratio. Optional node-label wrapping supports longer
  labels. The default `"legacy"` layout preserves historical geometry.
  Stored reports offer `reporting_layout = "compact"`, configurable row
  heights, and `panel_labels` for titles, axes and captions; figures and
  tables retain the same stored estimates, weights, thresholds and
  uncertainty metadata. Compact spacing also reaches nested depth-two
  projections, compact default heights follow the fitted rather than the
  requested depth, an empty decision-tree title removes the title row,
  `panel_labels = NULL` is accepted, and native trees take the model
  recorded on supplied leaf metrics. `edge_label_offset` is documented
  in x data units and `span_ratio` as legacy-only. Compact reports drop
  panel subtitles and move scope and comparator statements into
  captions, give the depth-two tree row more height, place compact edge
  labels on the upper part of each branch, and pad the contrast and gain
  axes so intervals do not span the full panel.

## \[2026-09-10\] margot 1.2.1

- Weighted policy projections now remove the point stroke so rendered
  circle areas remain proportional to display weights, including zero
  area for zero-weight records. Constant-tree projection subtitles
  honour custom action labels. SVG geometry tests cover stumps,
  depth-two trees and constant rules.

- Policy reporting can now bind stored leaf contrasts and policy-value
  gains to explicit rule, population, scale, weight and uncertainty
  metadata with
  [`margot_policy_reporting_data()`](https://go-bayes.github.io/margot/reference/margot_policy_reporting_data.md).
  Two standalone plots and matching text functions share these stored
  quantities. `margot_report_policy_tree(reporting_data = ...)` adds the
  tall A/B combo and side-by-side C/D evaluation panels. Missing
  intervals remain explicitly unavailable; reporting fits no model or
  interval. Legacy calls retain their existing calculations.

- Policy projections accept `display_weights` mapped to circular point
  area. Weighted displays retain exact predictor coordinates and use
  every reference row once across root branches, with a common area
  scale. The native combo now honours heights and A/B tags at both
  depths. Constant trees plot successfully, pruned child branches follow
  their stored links, and threshold-equal points occur only in the
  inclusive branch when shading filters unweighted projections. Existing
  unweighted jitter defaults remain unchanged.

- Depth-one policy projections now offer
  `jitter_method = "band_boundary"`: symmetric jitter bands remain
  centred on their predictor values, and the separator follows the
  inclusive band’s edge. The horizontal width is reduced uniformly when
  neighbouring groups would overlap, including on transformed axes.
  Fitted cut labels and predictions are unchanged; this option is
  forwarded through
...
- [`margot_plot_policy_tree()`](https://go-bayes.github.io/margot/reference/margot_plot_policy_tree.md)
  adds `jitter_method = "within_splits"` to keep each displayed point on
  its original side of every split, including values equal to the
  threshold. It supports transformed axes, explicit jitter widths and
  heights, and a reproducible seed.
  [`margot_plot_policy_combo()`](https://go-bayes.github.io/margot/reference/margot_plot_policy_combo.md)
  accepts these settings through `policy_tree_args`. Default
  `"standard"` jitter is unchanged and may cross cut lines; all action
  predictions still use the original covariates.

- [`margot_text_policy_tree()`](https://go-bayes.github.io/margot/reference/margot_text_policy_tree.md)
  can interpret stored cross-validation policy decisions and signed leaf
  summaries. It retains the training-selected comparator, checks stored
  arithmetic and the unrounded selection margin, and distinguishes
  leaf-score ranges from subgroup confidence intervals. It fits no model
  and computes no new policy value.

- Tree interpretations describe assigned actions rather than advice or
  recommendations. The bullet introduction no longer claims that splits
  establish treatment-effect modification. The interpreter follows
  stored child links, including pruned or constant trees, and no longer
  assumes a half-sample training split. Failed conditional-mean
  interpretation raises an error instead of inserting an editing message
  into public prose.
...
- Model, LMTP, and deprecated marginal-table E-values are now calculated
  from unrounded estimates, standard errors, and confidence limits.
  Combined tables retain their unrounded numeric values when they feed
  multiplicity-adjusted or multi-bias E-value calculations. Display
  rounding is deferred to presentation code and no longer changes the
  sensitivity calculation.

##### Changed

- Margot now computes ordinary risk-ratio E-values and the
  standardised-continuous-outcome approximation directly from the
  published equations. The `EValue` package is now an optional
  dependency used only for
  [`margot_multi_evalue()`](https://go-bayes.github.io/margot/reference/margot_multi_evalue.md)
  analyses that specify multiple biases.
...
- [`margot_lmtp_positivity_gate()`](https://go-bayes.github.io/margot/reference/margot_lmtp_positivity_gate.md)
  applies a pre-registered trim-ladder positivity decision rule to LMTP
  fits. At each rung (default 0.99, 0.98, 0.96) a policy passes when (1)
  the Kish effective sample size of the rung-winsorised cumulative
  weights clears a floor (default 50% of uncensored observations at
  every wave), (2) the winsorisation cap removes no more than a
  registered share of cumulative ratio mass (default 5%), and (3) the
  cumulative-product support status is not “Limited”. The two
...
- [`margot_lmtp_weight_diag_from_fit()`](https://go-bayes.github.io/margot/reference/margot_lmtp_weight_diag_from_fit.md)
  wave tables now report `trim_mass_share`, the share of cumulative
  ratio mass removed by the winsorisation cap at each wave (the
  bias-side quantity the gate budgets).
...
- [`margot_policy_workflow()`](https://go-bayes.github.io/margot/reference/margot_policy_workflow.md)
  now uses single-keyword defaults for `interpret_models` and
  `plot_models`, preventing the default call from treating preset
  choices as custom model names and silently skipping policy prose or
  plots.
...
- [`margot_select_grf_policy_trees()`](https://go-bayes.github.io/margot/reference/margot_select_grf_policy_trees.md)
  applies the current manuscript graphing rule for GRF policy trees:
  graph only selected policies whose policy-value and treated-uplift
  lower confidence limits exceed configurable thresholds.

#### Changed

- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  now defaults difference-scale plots to `x_lim_lo = -0.5`,
  `x_lim_hi = 0.5`, and `x_offset = -0.5`, giving standardised effect
  plots a compact default range and fixed coefficient-label position.

#### Fixed

- [`margot_grf_overlap()`](https://go-bayes.github.io/margot/reference/margot_grf_overlap.md)
  now handles saved causal-forest covariates stored as matrices and no
  longer emits the soft-deprecation notice intended for direct
  [`margot_assess_overlap()`](https://go-bayes.github.io/margot/reference/margot_assess_overlap.md)
  calls.
- GRF overlap summaries now extract the differential calibration
  estimate and p-value from
  [`grf::test_calibration()`](https://rdrr.io/pkg/grf/man/test_calibration.html)
  explicitly, instead of mislabelling the matrix’s second cell as a
  p-value.
...
- Deprecated the remaining qs-named storage helpers. `here_save_qs()`
  ...
  for general object storage.
- LMTP crash-recovery saves remain `.rds` files.
  `margot_lmtp(save_output = TRUE)` still writes per-model checkpoints
  immediately after each successful fit, which keeps partial work
  recoverable after crashes or power cuts without relying on `qs`,
  `qs2`, Arrow, or Docker.
...
- Soft-deprecated older `clarify`/matching workflows and moved `clarify`
  and `MatchThem` out of hard imports.

#### Dependency cleanup

- Retained `qs` in `Suggests` only, so fresh installs do not need it but
  older R environments can still convert archived `.qs` data directly.
- Removed `MatchIt`, `tidyverse`, `janitor`, `labelled`, `furrr`, and
  `Rdpack` from package dependency metadata.
- Kept `fastpolicytree`, `doParallel`, and `SuperLearner` as hard
  imports for the policy-tree and LMTP workflows.
...
- [`margot_plot_multi()`](https://go-bayes.github.io/margot/reference/margot_plot_multi.md)
  now skips an unnecessary first rendering pass when shared x-axis
  limits are supplied, while validating that the supplied limits are
  finite.
- Plot-related historical smoke scripts were moved out of the active
  test surface into `tests/archive/`, and `tests/testthat/` now carries
  a focused
  [`margot_interpret_marginal()`](https://go-bayes.github.io/margot/reference/margot_interpret_marginal.md)
  contract test for original-scale interpretation.

#### Fixed

- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  now uses the current ggplot2 horizontal error-bar path, removing the
  deprecation warnings from the stable plotting tests.
- Removed a deprecated string concatenation path in helper code that
  surfaced during the new interpretation test.
...
- Positivity reporting now labels the null shift as “Observed
  (Censoring-Adjusted)” instead of “Identity” when no custom label is
  supplied, in both `margot_pretty_positivity_shift()` and
  [`margot_report_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_report_lmtp_positivity.md).

#### Fixed

- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  Bonferroni reporting text now states the number of comparisons
  explicitly (e.g., “adjusted for 20 comparisons using Bonferroni
  correction”). Previously the text reported only “$`\alpha`$ = 0.05”,
  which could be misread as the unadjusted per-comparison threshold.
...
- Signal score methods text now includes clarification: “This
...
- Replaced Unicode special characters (em-dash, multiplication sign)
  ...
  output.
- Removed placeholder “Ehrlich et al., 2024” citation; policy value
...
- Policy value explanation in interpret batch uses consistent
  terminology with the policy explainer: “*Policy value*” and “*uplift*”
...
- Both bullet and prose report formats now distinguish “borderline
  positive” outcomes (CI lower bound \> -0.01 with positive point
  estimate) from truly “inconclusive” outcomes. This classification is
...
- Prose output now uses present tense (“yields”, “is”) rather than past
  tense (“yielded”, “was”) because inferences are made to the
  population, not the sample.
- Policy value explainer is now more concise: adds an introductory
  sentence (“The following terms are used throughout this report”),
  removes the technically problematic definition of confidence intervals
  (now simply states “All estimates are reported with 95% confidence
  intervals”), and tightens prose throughout.
...
- [`margot_policy_workflow()`](https://go-bayes.github.io/margot/reference/margot_policy_workflow.md):
  Renamed table columns from “Effect Size” to “Policy Value” and “Effect
  in Treated” to “Uplift in Treated” for consistency with policy
...
- Terminology now consistently uses “Policy Value” (the expected welfare
  gain from targeting vs a baseline) rather than “Effect Size”
...
- [`margot_policy_workflow()`](https://go-bayes.github.io/margot/reference/margot_policy_workflow.md)
  ...
  depth-2 policy values for all outcomes, along with the gain (Δ),
...
- Depth selection rationale is now explicitly documented: for **additive
  heterogeneity** (e.g., τ = α + β₁X₁ + β₂X₂), depth-1 trees are
...
- [`margot_positivity_report()`](https://go-bayes.github.io/margot/reference/margot_positivity_report.md)
  now returns `wave_summary_table` (shift × wave ESS per N% grid
...
- [`margot_lmtp_weight_diag_from_fit()`](https://go-bayes.github.io/margot/reference/margot_lmtp_weight_diag_from_fit.md)
  ...
  trimmed) even when the original censor columns aren’t available.
...
- [`margot_transition_ipsi_summary()`](https://go-bayes.github.io/margot/reference/margot_transition_ipsi_summary.md)
  ...
  with their exact Clopper–Pearson CIs.
- [`margot_compute_ipsi_probability()`](https://go-bayes.github.io/margot/reference/margot_compute_ipsi_probability.md)
  keeps raw probabilities plus 95% CIs, so downstream helpers don’t need
...
- [`margot_transition_ipsi_summary()`](https://go-bayes.github.io/margot/reference/margot_transition_ipsi_summary.md)
  ...
  LaTeX-ready narrative describing each wave pair’s natural rate plus
  the δ-specific counterfactual probabilities/fold increases.
- [`margot_compute_ipsi_probability()`](https://go-bayes.github.io/margot/reference/margot_compute_ipsi_probability.md)
...
- IPSI context (odds‑free) in
  [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md)
  with a simple probability‑scale formula and small illustrative
  translations; domain‑agnostic default labelling (“exposure”).
- Deterministic policy context describing history‑dependent rules (e.g.,
  A_t^d := d_t(A_t, H_t)) and listing included policies.
- Optional policy‑implied exposure rates by wave and overall using
  reweighted means; supports thresholding non‑binary exposures with an
...
- Wave labels in positivity text respect user `label_mapping` (e.g.,
  “Baseline (2018/19)”), supporting skipped panels.
- Clarified positivity vs target: diagnostics labelled “positivity;
  uncensored rows” with a note that estimation reweights to the baseline
  cohort via censoring adjustment.
- Per‑wave bullets include both `ESS+/(N+)` and `ESS+/(N_pt)`.
- Censoring wording clarified: “censoring to next wave” for intermediate
  waves, “censoring end of study” for the final exposure; shift
  summaries show “censoring (zeros across person‑time)”.
...
- Reverted
  [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  to the last known stable implementation (pre‑DEV) to resolve recent
...
- We are consolidating plotting on the experimental engine while we
  stabilise risk‑ratio E‑value logic and multiplicity adjustments.
  Further robustness improvements will ship in a follow‑up.
...
- **Fixed**: Set `mc.cores` and `parallelly.maxWorkers.localhost` early
  when `manage_future_plan = TRUE` to prevent “only 1 CPU cores
  available” errors with nested futures
...
- [`margot_lmtp()`](https://go-bayes.github.io/margot/reference/margot_lmtp.md):
  ...
  model can now use user’s future plan for parallel CV
- [`margot_lmtp()`](https://go-bayes.github.io/margot/reference/margot_lmtp.md):
...
- **With external future plan**: Set
  ...
  [`margot_lmtp()`](https://go-bayes.github.io/margot/reference/margot_lmtp.md)
  with default `manage_future_plan = FALSE` → ~5x faster (sequential
  models, parallel CV)
- **With managed nested futures**: Use
  `margot_lmtp(..., manage_future_plan = TRUE, cv_workers = 5, n_cores = 12)`
  → ~10x faster (2 models × 5 CV workers = 10 cores)
...
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
  ...
  `censoring = X%; uncensored ESS by wave — ...`.
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
...
- **LMTP density ratio plots now show only uncensored observations by
  ...
  “uncensored \| censored: X%” or “censored: X%” depending on parameter.
...
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
  methods section (`include_methods = TRUE`) now includes “Censoring
  vs. Treatment Positivity” subsection explaining the distinction and
...
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
  header now reads “LMTP positivity diagnostics for \[outcome\]
...
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
  ...
  [`margot_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_lmtp_positivity.md)
  and display “Censoring rate” separately.
...
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
  removed redundant “ESS computed from raw density ratios using colSums”
  text from header.
- [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md):
...
- [`margot_plot_lmtp_overlap_grid()`](https://go-bayes.github.io/margot/reference/margot_plot_lmtp_overlap_grid.md)
  ...
  `annotate_graph` parameter accepting “none”, “waves”, or “shifts”.
- [`margot_plot_lmtp_overlap_grid()`](https://go-bayes.github.io/margot/reference/margot_plot_lmtp_overlap_grid.md)
  and
  [`margot_lmtp_overlap_plot_grid()`](https://go-bayes.github.io/margot/reference/margot_lmtp_overlap_plot_grid.md):
  replaced `ymax_by_wave` with more flexible `ymax_harmonize` parameter
  accepting “none” (default), “row”, “column”, “global”, or custom named
  vectors.
...
- `xlim_harmonize` parameter for x-axis harmonization in
  ...
  and
  [`margot_lmtp_overlap_plot_grid()`](https://go-bayes.github.io/margot/reference/margot_lmtp_overlap_plot_grid.md)
  with options: “none” (default), “row”, “column”, “global”, or custom
  values.
- Automatic colour detection for shifts in
  [`margot_lmtp_overlap()`](https://go-bayes.github.io/margot/reference/margot_lmtp_overlap.md):
  shifts containing “null” → grey
  ([\#7](https://github.com/go-bayes/margot/issues/7)f7f7f), “up” →
  orange (#d95f0e), “down” → blue
  ([\#2](https://github.com/go-bayes/margot/issues/2)c7fb8).
- Layout-aware axis harmonization: “row” and “column” options
...
- [`margot_lmtp_overlap_plot_grid()`](https://go-bayes.github.io/margot/reference/margot_lmtp_overlap_plot_grid.md)
  now harmonises y‑axis heights across panels and accepts a `ymax`
...
- [`margot_lmtp()`](https://go-bayes.github.io/margot/reference/margot_lmtp.md)
  ...
  working; lmtp ‘listens’ to sequential parrallel setting, slowing its
...
- [`margot_policy_summary_report()`](https://go-bayes.github.io/margot/reference/margot_policy_summary_report.md)
  adds a “Practical Takeaways” block, per-model depth overrides
...
- [`margot_policy_summary_report()`](https://go-bayes.github.io/margot/reference/margot_policy_summary_report.md)
  outputs model id’s for wins/neutral/caution, to make reporting of
  policy trees easier
- [`margot_policy_summary_report()`](https://go-bayes.github.io/margot/reference/margot_policy_summary_report.md)
...
- [`margot_interpret_policy_tree()`](https://go-bayes.github.io/margot/reference/margot_interpret_policy_tree.md)
  ...
  subgroups. The narrative includes “Avg uplift among treated,” with a
  bootstrap CI.
...
- **Improved**
  ...
  now return ‘permitted_model_ds’ for use with policytree analysis.
...
- **Improved plot labeling for clarity**:
  - Removed “outcome” label from y-axis in
    [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
    for cleaner appearance
  - Changed “Qini Curves” to “Uplift Curve” in plot titles across all
    qini plot functions
  - Changed “average policy effect” to “expected policy effect” in
    y-axis labels to better reflect the statistical nature of the
    estimates
  - Updated documentation and scale notes to use consistent “expected
    policy effect” terminology
...
- **Refactored outcome flipping to happen before computation**:
  ...
  - Flipped outcomes are now consistently stored in the data with “\_r”
    suffix
...
- **Fixed model name detection for flipped outcomes in heterogeneity
  ...
  - Updated
    [`margot_interpret_heterogeneity()`](https://go-bayes.github.io/margot/reference/margot_interpret_heterogeneity.md)
    to handle model names with “\_r” suffix (flipped outcomes)
  - Updated
    [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
    to handle model names with “\_r” suffix and simplified Y data
    extraction
  - Updated
  ...
  - Now correctly maps requested names like
    “t2_kessler_latent_anxiety_z” to
    “model_t2_kessler_latent_anxiety_z_r”
  - Fixes issue where functions couldn’t find flipped models when using
...
- This prevents stray “\_r” suffixes appearing on plot labels when using
  label_mapping
...
- **Reimplemented QINI curves following GRF best practices**:
  ...
  - **Fixed DR scores consistency**: QINI curves now use doubly-robust
    ...
    - Addresses discrepancy between QINI curve termination values and
      reported ATE
    - Following GRF advice: “use the same doubly-robust scores
      throughout”
...
- **Updated default baseline_method to “auto” in plotting functions**:
  - Changed
    ...
    default from “maq_no_covariates” to “auto”
...
- **Simplified interpretation output for log-transformed outcomes**:
  ...
  - Confidence intervals now displayed in original scale units (dollars,
    minutes, etc.)
  - Cleaner format: “\$463 average increase (95% CI: \$137 to \$883)”
    instead of “44% multiplicative increase (~\$463 average increase;
    95% CI: 13% to 84%)”
  - Updated both
    [`margot_interpret_marginal()`](https://go-bayes.github.io/margot/reference/margot_interpret_marginal.md)
    and
...
- **Updated
  ...
  - Matches the sophisticated back-transformation logic from
    [`margot_interpret_policy_tree()`](https://go-bayes.github.io/margot/reference/margot_interpret_policy_tree.md)
  - Example: “47% multiplicative increase (~\$490 average increase; 95%
    CI: 32% to 64%)”
...
- **Corrected interpretation of log-transformed outcomes**:
  ...
  - Example: CATE of 0.14 with log SD of 2.74 gives 47% multiplicative
    increase
  - Updated display format: “47% multiplicative increase, ~\$490 average
    increase”
...
- **Improved reporting for log-transformed outcomes**:
  ...
  - Log-transformed monetary outcomes now show percentage change and
    dollar values
  - Example: “13% increase, from \$45 to \$51” instead of “1.13x
...
- **Display CATEs on original data scale in policy tree
  ...
  - When original data is provided, CATEs are shown in both standardized
    and original scales
  - For z-transformed outcomes: displays as “CATE: 0.1 (original scale:
    0.5 units)”
  - For log-transformed outcomes: displays as “CATE: 0.1 (original
    scale: 1.2x multiplicative effect)”
...
- **Enhanced label mapping in
  ...
  - Improved default transformation to remove wave prefixes (e.g.,
    “t2\_”)
  - Now correctly identifies reversed variables (ending in “\_r” or
    “\_z_r”) and adds “(reduced)” prefix
  - Added special handling for common abbreviations (hlth –\> health,
    bmi –\> BMI, pwi –\> PWI)
  - Results in cleaner outcome names like “Belonging” instead of “T2
    Belong”
  - Reversed variables now show as “(reduced) Health Fatigue” instead of
    “T2 Hlth Fatigue z r”
- **Updated to use New Zealand English spelling throughout**:
  - Changed “analyze” –\> “analyse”, “analyzed” –\> “analysed”,
    “organize” –\> “organise”
...
- **Cross-validation now provides consistent estimates in
  ...
  - Confidence intervals are no longer reported for CV estimates as they
    don’t align with the martingale-based hypothesis testing
...
- **Clearer decision flow and reporting**:
  ...
  - Simplified recommendations format: “Use for targeting: X”, “Avoid
    targeting: Y”, “Evidence unclear: Z”
...
- **Fixed
  ...
  - Updated all references from “bootstrap” to “stability” in
    documentation and messages
- **Fixed incorrect “positive RATE” labeling in
  [`margot_interpret_heterogeneity()`](https://go-bayes.github.io/margot/reference/margot_interpret_heterogeneity.md)**:
  - Function now correctly identifies and labels negative RATE values
  - Properly handles mixed positive/negative values (e.g., “positive
    RATE AUTOC, but negative RATE QINI”)
...
- **Fixed misleading omnibus test language**:
  - Extended reports no longer say “confirmed by omnibus tests” when the
    omnibus p-values are not significant
  - Now uses neutral language “Omnibus tests:” for non-significant
    results
- **Clarified decision flow language**:
  - Removed confusing “Avoid Y”, “Use X”, “Evidence unclear for Z”
    placeholders
  - Now clearly states actions in terms of targeting specific outcomes
  - Updated recommendations to use “Outcomes suitable for targeted
    treatment” instead of “Use for targeting”
...
- **Renamed bootstrap functions to stability functions**:
  ...
  - Class `"margot_bootstrap_policy_tree"` –\>
...
- **More precise negative RATE detection in
  ...
  - Only models with significant negative RATE (not just negative
    estimates) are flagged as “excluded_negative_rate”
  - Inconclusive results are no longer misclassified as negative
  - Enhanced recommendations to distinguish between:
    - “AVOID targeting X” (statistically significant negative RATE -
      harmful)
    - “Insufficient evidence for heterogeneity in Y” (no evidence found)
    - “Statistical heterogeneity detected but no actionable targeting
      evidence for Z” (omnibus positive but RATE not positive)
...
- **Enhanced recommendations in
  ...
  - Provides clearer guidance: “Avoid targeting X due to negative RATE
    evidence” vs “No heterogeneity evidence found for Y”
...
- **Revised heterogeneity evidence classification in
  [`margot_interpret_heterogeneity()`](https://go-bayes.github.io/margot/reference/margot_interpret_heterogeneity.md)**:
  - Removed “mixed_evidence_caution” category for models with
    conflicting RATE results
  - Any model with negative RATE evidence (AUTOC or QINI) is now
    excluded entirely
  - Added new “exploratory_evidence” category for models with no
  ...
  - New evidence_type values: “excluded_negative_rate”,
    “exploratory_evidence” (replaces “mixed_evidence_caution”)
...
- **Fixed label mapping in CV rate tables**:
  ...
  - CV rate tables now properly display transformed model names (e.g.,
    “Personal Well-being Index” instead of “model_t2_pwi_z”)
...
- **Updated citations to distinguish methodologies**:
  ...
  - Nie & Wager (2020) cited for general RATE (Rank Average Treatment
    Effect) approach
  - Fixed citation from incorrect “2021” to correct “2020” for Nie &
...
- **Enhanced heterogeneity analysis with better defaults and CV
  ...
  - Changed default `adjust = "none"` to work better with CV (only
    “bonferroni” or “none” are valid for CV)
  - Enhanced
    [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
...
- **Changed default `tree_method` to “fastpolicytree” in
  [`margot_policy_tree_bootstrap()`](https://go-bayes.github.io/margot/reference/margot_policy_tree_bootstrap.md)**:
...
- **Added `model_names` parameter to
  ...
  - Model names can be specified with or without “model\_” prefix
...
- **Added automatic labelling to diagnostic functions**:
  - Enhanced
    ...
    to handle `_r` suffix (converts to “(reduced) {var_name}”)
  - Enhanced
    [`transform_var_name()`](https://go-bayes.github.io/margot/reference/transform_var_name.md)
    to handle `_log` pattern (converts to “{var_name} (log)”)
  ...
  - Model names in output are also transformed (e.g.,
    “model_t2_kessler_latent_depression_z_r” becomes “(reduced) Kessler
    Latent Depression”)
...
- **Added support for fastpolicytree package**:
  - New `tree_method` parameter in
    ...
    and
    [`margot_policy_tree_bootstrap()`](https://go-bayes.github.io/margot/reference/margot_policy_tree_bootstrap.md)
  - Allows users to choose between “policytree” (default) and
    “fastpolicytree”
...
- **Enhanced CLI output for bootstrap analysis**:
  - Now shows “seed variation (fixed train proportion)” for default
    behavior
  - Shows “seed + train proportion variation” when varying train
    proportions
...
- **Fixed misleading CLI output in
  [`margot_policy_tree_bootstrap()`](https://go-bayes.github.io/margot/reference/margot_policy_tree_bootstrap.md)**:
  - Now correctly shows “sample_only” when
    `vary_train_proportion = FALSE` (the default)
  - Previously showed “both” even when only bootstrap resampling was
    happening
...
- **New
  ...
  - Three variation types: “both” (resample + split), “sample_only”,
    “split_only”
  ...
  - Covariate selection modes: “original”, “custom”, “add”, “all” -
    identical to
    [`margot_policy_tree()`](https://go-bayes.github.io/margot/reference/margot_policy_tree.md)
  - Pattern-based exclusion (e.g., exclude all “\_log” variables)
  - **Changed default depth from “both” to 2** (most commonly used in
    practice)
...
- **Progress tracking for
  [`margot_interpret_heterogeneity()`](https://go-bayes.github.io/margot/reference/margot_interpret_heterogeneity.md)**:
  - Shows overall progress through major analysis steps (e.g., “\[2/4\]
    Computing QINI curves”)
...
- **Updated pkgdown documentation**:
  ...
  - Added `margot_plot_cv_results` and `margot_plot_cv_summary` to
    “Visualise Causal Effect Estimates” section
  - Marked `print.margot_cv_results` as internal (S3 method)
- **Better cross-validation integration in
  [`margot_interpret_heterogeneity()`](https://go-bayes.github.io/margot/reference/margot_interpret_heterogeneity.md)**:
  - Automatically converts invalid adjustment methods to “none” when
    using CV
  - No longer shows warning when default “BH” is automatically converted
    for CV
...
- **Fixed progress bar error in
  [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)**:
  - Progress bars now use explicit IDs to avoid “Cannot find current
    progress bar” errors
  - Properly handles progress bar lifecycle with NULL checks
- **Fixed parallel processing error “could not find function
  rate_sequential_cv”**:
  ...
  - Clear warning message guides users to either install the package or
    use `parallel = FALSE`
  - This resolves the “could not find function” error when using
    `parallel = TRUE`
...
- **Multiple testing correction defaults changed**:
  - [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
    now defaults to `adjust = "none"` (was “BH”)
  - [`margot_rate()`](https://go-bayes.github.io/margot/reference/margot_rate.md)
    now defaults to `adjust = "none"` (was NULL)
  - [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
    now only accepts “bonferroni” or “none” for `adjust` parameter
...
- **Flexible confidence intervals**:
  - `show_ci` parameter in QINI plots now accepts: FALSE, TRUE, “both”,
    “cate”, or “ate”
...
- **Budget-based visualization**:
  - New `x_axis` parameter in QINI functions accepts “proportion” or
    “budget”
...
- **Cross-validation heterogeneity testing**:
  - New
    [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
    function implementing GRF’s uncorrelated sequential CV approach
...
- **Multiple testing correction updates**:
  - Changed default `adjust` parameter to “none” in both
    [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
    and
    [`margot_rate()`](https://go-bayes.github.io/margot/reference/margot_rate.md)
  - Restricted
    [`margot_rate_cv()`](https://go-bayes.github.io/margot/reference/margot_rate_cv.md)
    to only accept “bonferroni” or “none” for statistical validity
...
- Fixed “invalid ‘type’ (closure)” error in
  [`margot_rate()`](https://go-bayes.github.io/margot/reference/margot_rate.md)
...
- Fixed treatment_cost NULL initialization causing QINI generation
  failures
- Fixed “Models not found” error in
  [`margot_plot_qini_batch_cost_sensitivity()`](https://go-bayes.github.io/margot/reference/margot_plot_qini_batch_cost_sensitivity.md)
- Fixed “\$ operator is invalid for atomic vectors” error by creating
  [`margot_plot_qini_direct()`](https://go-bayes.github.io/margot/reference/margot_plot_qini_direct.md)
- Fixed “invalid ‘x’ type in ‘x && y’” error by using proper boolean
  variables
...
- Fixed evidence type classification in
  ...
  - Added new “exploratory_only” category for models with only QINI
    curve evidence
...
- **Scale transformations for QINI plots**: Added `scale` parameter to
  ...
  and
  [`margot_plot_qini_batch()`](https://go-bayes.github.io/margot/reference/margot_plot_qini_batch.md)
  - “average” (default): Shows average policy effects per unit (maq’s
    Q(B) = E\[⟨πB(Xi), τ(Xi)⟩\])
  - “cumulative”: Shows traditional cumulative gains (total accumulated
    benefit)
  - “population”: Shows total population impact (absolute gain in
    outcome units)
  - Example:
...
- **Fixed QINI dimension mismatch with qini_split = TRUE**: Resolved
  critical error where tau_hat dimensions didn’t match test indices
  ...
  - Prevents “reward, costs, and evaluation scores should have
    conformable dimension” errors
- **Fixed rbind errors in QINI plots**: Resolved “numbers of columns of
  arguments do not match” errors
...
- **Consistent QINI baseline method**:
  [`margot_causal_forest()`](https://go-bayes.github.io/margot/reference/margot_causal_forest.md)
  now uses “maq_no_covariates” as default
...
- **QINI data consistency**: Ensured average gain calculations use the
  ...
  - Fixed baseline_method metadata to correctly reflect “maq_constant”
...
- **QINI baseline method consistency**: Enhanced tracking of baseline
  ...
  - Users get informative messages instead of cryptic “Cannot find
    outcome data” errors
...
- **margot_qini()**: new function for generating QINI curves and
  ...
  - supports all baseline methods: “auto”, “simple”,
    “maq_no_covariates”, “maq_only”, “none”
...
- **baseline_method**: changed default from “auto” to
  “maq_no_covariates” in all QINI functions
  - provides more theoretically sound default behavior
  - “maq_no_covariates” now includes automatic fallback to simple
...
- **flipped model data lookup**: fixed issue where QINI functions
  couldn’t find outcome data for flipped models
  - models with “\_r” suffix (e.g., “model_t2_neuroticism_z_r”) now
...
- **margot_interpret_heterogeneity()**: improved clarity of test results
  - renamed “omnibus_test” column to “differential_prediction_test” for
    clarity
  - added “mean_prediction_test” column to show calibration status
    (informational only)
...
- **margot_plot_qini() enhancements**:
  ...
  - added baseline_method parameter for flexible baseline generation
    (“auto”, “simple”, “maq_no_covariates”, “maq_only”, “none”)
  ...
  - smart fallback logic: adds simple baseline to existing QINI data
    when regeneration isn’t possible
...
- **margot_plot_qini_batch()**: Batch process and plot QINI curves for
  ...
  - supports all margot_plot_qini() parameters for consistent
    visualization
  - handles model names with or without “model\_” prefix
...
- Fixed
  ...
  - Resolved “Couldn’t find a point-estimate column” error that occurred
...
- Fixed double “model\_” prefix bug in
  [`margot_flip_forests()`](https://go-bayes.github.io/margot/reference/margot_flip_forests.md)
  - Results now correctly show as “model_outcome_r” instead of
    “model_model_outcome_r”
...
- **margot_recompute_ate()**: Recompute average treatment effects with
  ...
  - Allows recomputation without refitting expensive causal forest
    models
  - Supports target_sample options: “all” (ATE), “treated” (ATT),
    “control” (ATC), “overlap” (ATO)
...
- **New rename_evalue Parameter**:
  - When TRUE, renames E_Value to “E-Value” and E_Val_bound to “E-Value
    Bound”
...
- **margot_interpret_marginal()**:
  ...
  - Explicitly states “No reliable effects are evident.” when no
...
- **Reversed outcome formatting**:
  - Updated
    ...
    to prepend rather than append “(reduced)”
...
- **Policy tree interpretation improvements**:
  - Modified
    [`margot_interpret_policy_batch()`](https://go-bayes.github.io/margot/reference/margot_interpret_policy_batch.md)
    to remove redundant “Policy tree analysis results:” line
  - Added “at the end of study” to all Findings headings for temporal
    clarity
  - Added “baseline” prefix to all variable names in tree splits and
    leaf descriptions
...
- **Policy tree plotting updates**:
  - Enhanced
    [`margot_plot_policy_tree()`](https://go-bayes.github.io/margot/reference/margot_plot_policy_tree.md)
    to add “(baseline)” to all axis labels
  ...
  - Enhanced
    [`margot_plot_decision_tree()`](https://go-bayes.github.io/margot/reference/margot_plot_decision_tree.md)
    to append “Outcome” to outcome titles
  - Example: “Neuroticism” now displays as “Neuroticism Outcome”
...
- Enhanced
  ...
  - Added `output_format` parameter: “bullet” (default) or “prose” for
...
- [`margot_flip_forests()`](https://go-bayes.github.io/margot/reference/margot_flip_forests.md)
  completely rewritten to create new models with “\_r” suffix
...
- Updated
  [`margot_reversed_labels()`](https://go-bayes.github.io/margot/reference/margot_reversed_labels.md)
  to handle new “\_r” suffix convention
...
- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  ...
  corrected – so the interpretation text and interpretation table match.
...
- `margot_plot_policy_tree_depth2()`,
  ...
  make panels gray – just a quirk – not an issue for final graph
  project.)
...
- [`margot_reversed_labels()`](https://go-bayes.github.io/margot/reference/margot_reversed_labels.md)
  – helper that gets display labels and marks reversed outcomes (for
  policy trees)
- [`margot_interpret_rate()`](https://go-bayes.github.io/margot/reference/margot_interpret_rate.md)
  ([`margot_interpret_rate_comparison()`](https://go-bayes.github.io/margot/reference/margot_interpret_rate_comparison.md))-
...
- `margot_planned_subgroup_batch()` and
  ...
  – numerous enhancements for clear reporting
...
- [`margot_process_longitudinal_data_wider()`](https://go-bayes.github.io/margot/reference/margot_process_longitudinal_data_wider.md)
  correctly creates “NA” value for censoring indicator when unit is lost
...
- [`margot_correct_combined_table()`](https://go-bayes.github.io/margot/reference/margot_correct_combined_table.md)
  does family–wise-error correction for multiple outcomes (ATE)
- `margot_add_policy_values()` - attach policy values to forest models
  (utility)
- `margot_collect_policy_values()` – gather policy values from forest
  models
...
- [`margot_plot_decision_tree()`](https://go-bayes.github.io/margot/reference/margot_plot_decision_tree.md)-
  colours robustly match decisions ‘control’, ‘treat’
...
- [`margot_rate_batch()`](https://go-bayes.github.io/margot/reference/margot_rate_batch.md) -
  Two allocation rules are available: **“treat_best”** (default) ─
  administer treatment to those with the largest predicted benefits;
  **“withhold_best”** ─ *remove* treatment from that same subgroup,
  useful when the intervention harmful. The CATE vector is flipped
  (multiplied by −1) *after* model fitting and *before* calling
  [`grf::rank_average_treatment_effect()`](https://rdrr.io/pkg/grf/man/rank_average_treatment_effect.html).
...
- [`margot_wide_machine()`](https://go-bayes.github.io/margot/reference/margot_wide_machine.md)
  now handles allows for `imputation_method` = “none”. new parameters
...
- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md) -
  added options to transform colname for the estimand to “ATE”, also the
  E_value colnames.
- [`margot_interpret_marginal()`](https://go-bayes.github.io/margot/reference/margot_interpret_marginal.md)
  and
...
- [`margot_lmtp_tab()`](https://go-bayes.github.io/margot/reference/margot_lmtp_tab.md)
  – fixes breaking change introduced by `lmtp v1.50`
...
- [`margot_transition_table()`](https://go-bayes.github.io/margot/reference/margot_transition_table.md)
  – allows filtering by observation/censoring variable, can be useful
  where data are entirely missing from certain waves.
- [`margot_bind_tables()`](https://go-bayes.github.io/margot/reference/margot_bind_tables.md)
  – correct output retaining outcome names
- [`margot_impute_carry_forward()`](https://go-bayes.github.io/margot/reference/margot_impute_carry_forward.md)
  – better error handling/ if observation is seen in a year, may impute
  forward.
...
- fixed multiple functions to get assending order for
  ...
  [`group_tab()`](https://go-bayes.github.io/margot/reference/group_tab.md),
  [`margot_interpret_marginal()`](https://go-bayes.github.io/margot/reference/margot_interpret_marginal.md)
  `transform_label()`, `transform_table_rownames()` – and removing stray
...
- [`margot_rate()`](https://go-bayes.github.io/margot/reference/margot_rate.md)
  output better worded, still reate “rate_result” for backward
  compatibility; table only highlight “significant” if RATE is positive.
- [`margot_interpret_qini()`](https://go-bayes.github.io/margot/reference/margot_interpret_qini.md) -
  better outputs
...
- [`margot_interpret_qini()`](https://go-bayes.github.io/margot/reference/margot_interpret_qini.md)
  and
  [`margot_interpret_rate()`](https://go-bayes.github.io/margot/reference/margot_interpret_rate.md)
  play better with LaTeX. Use “statistically reliable” in place of
...
- [`margot_censor()`](https://go-bayes.github.io/margot/reference/margot_censor.md) -
  explicit call for ‘cli’ to avoid conflict with crayon
...
- [`margot_causal_forest()`](https://go-bayes.github.io/margot/reference/margot_causal_forest.md)
  ...
  compatible – no modifications to user code are required – and a note
...
- [`margot_planned_subgroups_batch()`](https://go-bayes.github.io/margot/reference/margot_planned_subgroups_batch.md)
  – batch the batched analysis of planned subgroups analysis for causal
  forests (time-saver).
...
- `margot_process_binary()`: if a variable already ends with “\_binary”
...
- added
  ...
  helper function for strict dyadic censoring – now encompassed by
  [`margot_process_longitudinal_data_wider()`](https://go-bayes.github.io/margot/reference/margot_process_longitudinal_data_wider.md)
...
- [`margot_censor()`](https://go-bayes.github.io/margot/reference/margot_censor.md) -
  specified functions required by data.table (#’
...
- [`create_ordered_variable()`](https://go-bayes.github.io/margot/reference/create_ordered_variable.md)
  appends “\_binary” when binary variables are made
- [`margot_transition_table()`](https://go-bayes.github.io/margot/reference/margot_transition_table.md)
...
- `margot_process_longitudinal_data_wider` can now handle dyadic data,
  so that if an agent’s partner is censored, then the agent is also
  censored.
...
Standardised, Standardized, or to remove it altogether (“none”). - For
Risk Ratios (type = “RR”), the code always omits that word and shows the
label as “Effect (Risk Ratio Scale)”. - For Risk Differences (type =
“RD”), if standardize_label = “NZ” you get “Standardised Effect
(Difference Scale)”; if standardize_label = “US” you get “Standardized
Effect (Difference Scale)”; and if standardize_label = “none”, you get
...
observation is made and ‘require_one_observed’ is set to “TRUE”.
...
- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  label now reads “causal difference” rather than “causal risk
  difference”.
...
handling ‘prodigal’ id’s that are lost and found).
...
- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md),[`margot_interpret_marginal()`](https://go-bayes.github.io/margot/reference/margot_interpret_marginal.md),
  ...
  standardised effects to effects on the data scale – greatly benefiting
  interpretations.
...
- `margot_plot_save_png()` saves a margot_plot output graph as a png,
  user can change width, heigh, dpi, and specify a path…
...
- [`margot_summary_cate_difference_gain()`](https://go-bayes.github.io/margot/reference/margot_summary_cate_difference_gain.md)
  ...
  specified spend level – to see if there is support for CATEs
...
- [`margot_plot_histogram()`](https://go-bayes.github.io/margot/reference/margot_plot_histogram.md)
  new histogram that’s more informative and more robust than previous
...
- `margot_merge_databases()` is deprecated. Use
  ...
  ‘databases’ in the new function name.)
...
- `margot_create_database` made more robust, menus work better, and you
  can copy information from other citations. (It’s clear this will need
  to be a separate package,… expect a move down the track)
...
- `margot_plot_decision_tree` - user may remove ’Action” labels for
...
- `manager_boilerplate_measures` now flexibly handles strings for
  references when ‘string_is …’ is used in the reference category that
...
- Shared text helper
  ...
  coverage, uplift, and the identity PV(control-all) = Coverage ×
  Uplift_Treated.
...
- [`margot_plot_lmtp_overlap_grid()`](https://go-bayes.github.io/margot/reference/margot_plot_lmtp_overlap_grid.md)
  enforces robust defaults — `layout = "shifts_by_waves"` and
  `color_by = "shift"` — and prints a brief CLI info message when
...
- Added `signal_score` (“pv_snr”, “uplift_snr”, “hybrid”) and
  `signals_k`:

  - Renders a “Signals Worth Monitoring” section ranking Neutral models
  ...
  - Adds a programmatic `signals_df_all` with a full ranking across all
    models
  - Falls back to ranking non‑wins when Neutral is empty, ensuring
...
- Present‑tense, general prose in interpretation; moved harm flags into
  a separate Risk Notes section

- Soft‑deprecate Neutral in policy audience (configurable via
  `show_neutral`)

- `prefer_stability` biases depth selection toward depth‑1 unless
  depth‑2 gains are clearly larger

- Acronym expansion opt‑in: `expand_acronyms` with user‑overrides via
  options \# \[2025-10-22\] margot 1.0.264 \### Added

- IPSI context (odds‑free) in
  [`margot_interpret_lmtp_positivity()`](https://go-bayes.github.io/margot/reference/margot_interpret_lmtp_positivity.md)
  with a simple probability‑scale formula and small illustrative
  translations (domain‑agnostic; defaults to “exposure”).

- Deterministic policy context describing history‑dependent rules (e.g.,
  A_t^d := d_t(A_t, H_t)) and listing included policies.

- Optional policy‑implied exposure rates by wave and overall using
  reweighted means; non‑binary exposures are thresholded via an
  indicator 1(A_t op tau) (defaults: op is \>, tau = 0).
...
- Experimental
  ...
  [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  reverts to the pre‑multiplicity API.

- [`margot_plot()`](https://go-bayes.github.io/margot/reference/margot_plot.md)
  now delegates to
  [`margot_plot_dev()`](https://go-bayes.github.io/margot/reference/margot_plot_dev.md)
  under the hood, so existing scripts gain advanced options without code
  churn. We will continue to stabilise RR/E‑value handling.
...
- We now route
  ...
  stabilise RR/E‑value logic. Further improvements to robust RR E‑values
  and multiplicity adjustments will ship in a follow‑up.
