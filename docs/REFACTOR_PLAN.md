...
Refactor margot to reduce Imports from 54 to ~15 packages while
maintaining all functionality for active users. This will be done
...
4.  **Lab workflow priority** - Teaching and research use cases come
    first
...
**Note**: Rather than creating a single `margot.models` meta-package, we
will create focused packages `margot.lmtp` and `margot.grf` for our core
causal inference methods. Traditional parametric approaches are
available through LMTP for comparison purposes.
