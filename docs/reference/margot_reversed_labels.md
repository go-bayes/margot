...
This function now handles the new "\_r" suffix convention for flipped
models.
...
- use_r_suffix:
  ...
  logical; if TRUE (default), creates new entries with "\_r" suffix for
  flipped outcomes. If FALSE, uses the old behavior of modifying
  in-place.

- remove_original:

  logical; if TRUE and use_r_suffix is TRUE, removes the original
