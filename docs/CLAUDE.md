...
The `margot` package provides functions for **MARG**inal
**O**bservational **T**reatment-effects. It streamlines causal inference
workflows for psychological and social scientists, with a focus on
evaluating causal assumptions, modelling time-series data, reporting
results, and performing sensitivity analyses.
...
1.  **Data Processing Functions** (`R/margot_process_*.R`,
    `R/margot_wide_*.R`)
...
2.  **Causal Inference Models**
    - [`margot_causal_forest()`](https://go-bayes.github.io/margot/reference/margot_causal_forest.md):
      Main causal forest implementation (grf-based)
    - [`margot_lmtp()`](https://go-bayes.github.io/margot/reference/margot_lmtp.md):
      Modified treatment policy estimation
...
3.  **Visualisation System** (`R/margot_plot_*.R`)
    - Unified plotting interface via
    ...
    - Specialised plots:
      - [`margot_plot_qini()`](https://go-bayes.github.io/margot/reference/margot_plot_qini.md):
        Qini curves for treatment prioritisation
...
- Parallel processing variants exist for computationally intensive
  operations (e.g.,
  [`margot_causal_forest_parallel()`](https://go-bayes.github.io/margot/reference/margot_causal_forest_parallel.md))
...
The package expects longitudinal data in wide format with time-prefixed
variables: - Baseline: `t0_variable` - Follow-ups: `t1_variable`,
`t2_variable`, etc. - ~~The included `df_nz` dataset demonstrates the
expected structure~~ We will remove `df_nz` from the package and move it
to Zenodo. Data can now be fetched using
[`fetch_margot_data()`](https://go-bayes.github.io/margot/reference/fetch_margot_data.md)
and converted to a quickstart object using
[`here_read_qs()`](https://go-bayes.github.io/margot/reference/here_read_qs.md)
function. - ID variable should be named `id` - Sample weights:
`sample_weights` or time-specific weights
...
- Test coverage is currently minimal - new functions should include
  tests
- Many functions support batch processing for multiple
  outcomes/exposures
...
- **Todo List**: Maintain an ongoing list of tasks to be completed for
  the margot package
- **Completed Work List**: Track and record completed tasks, milestones,
  and achievements

### Testing Workflow

- Default to writing unit tests for all functions using testthat, and
  storing them in the tests folder
- ~~Use the included `df_nz` dataset for testing examples~~ Use
  [`fetch_margot_data()`](https://go-bayes.github.io/margot/reference/fetch_margot_data.md)
  to get test data
...
4.  Include progress bars for long-running operations using `cli` or
    `progressr`
